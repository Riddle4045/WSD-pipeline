 ...
 *	2) Disambiguate the same word for new sentenses:    uwsd -test /path/to/classifer text-to-be-classified word-to-be-disambiguated
 * Description:
...
void disambiguate_text(const vector< vector<string> >& text, int* label, const DECLIST& decisionList, double* confidence, int* pos_of_ambword, float* extend){ //extend = NxK confidence matrix.
    ...
    int i,j,k,l,m;
    int N = text.size();  ///number of testing instances.
    int K = num_senses;  ///number of senses.
    memset(extend,0,sizeof(float)*N*K);
        ...
        for(j=0;j<decisionList.pos.size();j++){ //test each rule.
            if(decisionList.pos[j]==-1){ //left immediate
                if(pos_of_ambword[i]>0 && text[i][pos_of_ambword[i]-1]==decisionList.determinants[j]){
                    ...
                    if(extend[i*K+decisionList.prediction[j]-1]<decisionList.conf[j])
                    	extend[i*K+decisionList.prediction[j]-1] = decisionList.conf[j];
                }
            }
...
int main(int argc, char** argv){
    ...
    if(strcmp("-train",argv[1])==0){
        ...
        if(argc!=5){
            puts("Incorrect number of parameters for testing mode.");
            return -1;
        }else{
            /*test*/
            ...
            num_senses = 0;
            for(i=0;i<decisionList.prediction.size();i++){
            	if(decisionList.prediction[i]>num_senses)
            		num_senses = decisionList.prediction[i];
            }
            float* extend = new float [text.size()*num_senses];
            disambiguate_text(text, label, decisionList, confidence, pos, extend);
            fclose(fp_classifer);
            //score normalization.
            float sum = 0;
            for(i=0;i<text.size();i++){
            	...
            	for(j=0;j<num_senses;j++)
               		sum += extend[i*num_senses+j];
               	if(sum>1e-6)
               	for(j=0;j<num_senses;j++)
               		extend[i*num_senses+j] /= sum;
               	else
               	for(j=0;j<num_senses;j++)
               		extend[i*num_senses+j] = 1.0/num_senses;
            }
            //print results.
            for(i=0;i<text.size();i++){
            	for(j=0;j<num_senses;j++){
            		if(j==num_senses-1)
            			printf("%.4f",extend[i*num_senses+j]);
            		else
               			printf("%.4f ",extend[i*num_senses+j]);
               	}
               	if(i>0)
